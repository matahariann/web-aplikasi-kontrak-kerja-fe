...
const ContractsForm = ({ currentStep, setCurrentStep }) => {
  ...
  const handleContractsSubmit = async (
    ...
    try {
        ...
        for (let i = 0; i < contractsData.length; i++) {
          ...
          try {
            if (i < savedContractsIds.length) {
              ...
              const response = await updateContract(token, oldId, formattedContract);
              if (response?.data?.id) {
      ...
      // Update localStorage
      localStorage.setItem(STORAGE_KEYS.SAVED_CONTRACTS_IDS, JSON.stringify(newSavedIds));
      localStorage.setItem(STORAGE_KEYS.CONTRACTS_DATA, JSON.stringify(contractsData));
      localStorage.setItem(STORAGE_KEYS.IS_CONTRACTS_SAVED, JSON.stringify(true));
      localStorage.setItem(STORAGE_KEYS.IS_CONTRACTS_EDIT_MODE, JSON.stringify(false));
